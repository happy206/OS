...
load_icode(unsigned char *binary, size_t size) {
    ...
    //在二进制数据空间中分配内存(虚拟、物理内存空间)，从elf格式的二进制程序中复制出对应的代码/数据段，并初始化BSS段(需要对ELF格式的文件有一定的了解)。
    struct Page *page;
    ...
    //建立新的vma
     ...
     //分配内存，把所有的programme section复制到进程内存当中
    ...
    //为了让当前的应用进程在加载之后可以顺利的回到用户态执行，需要构造中断栈帧。在中断返回后欺骗CPU，让CPU以为当前线程在之前发生了一次特权切换的中断，但事实上已经是一个新的程序了。
